...
It is still unclear if problematic pointer manipulation is currently UB
in Rust, but community discussions suggest that it might be in
...
(without exhibiting UB) in the XOR linked list.
}
...
\section{Capability sealing to protect objects in Safe Rust}
\todo{Implement a `referable by FFI/unsafe' trait to prevent unsafe/ffi
code from introspecting objects too far.}
...
\subsection{Enforcement of immutability across FFI boundaries}
...
\subsection{Sealed capabilities and callbacks}
...
\subsection{Optimisation in Safe Rust using hybrid mode}
