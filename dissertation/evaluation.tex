...
\todo{
Note that the non-capability implementation would use a 64-bit pointer
and a 64-bit variable, but the capability coprocessor speeds up
operations (vague; cite/remove?)
}
...
\todo{Transfer overhead (also see footprint reduction) -- positive but unpredictable?, tag overhead -- less than 5\% pretty much all situations}
...
\todo{
Unsafety exists because compiler analysis is too strong: want to allow
correct but unprovable things.
One of the reasons is that creating UB is easy with unsafe code: not
supposed to be possible in Safe Rust.
Capabilities can rule out 2/9 sources of UB by trapping rather than doing
unexpected things, improving safety (only spatial! dangling temporal
pointers still an issue)
This is explicitly safe:
https://doc.rust-lang.org/nomicon/what-unsafe-does.html
Like CHERI, Safe Rust allows oob pointers, as long as they're not
dereferenced.
This doesn't stop optimisation, since dereferencing dangling
pointers/mutating immut data is UB anyway.
}


\section{Limitations}
\todo{128-bit usize means no sensible evaluation of generated
instructions, no opt means no eval of runtimes. More?}
